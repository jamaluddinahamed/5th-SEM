...
#------------------Dependencies initializations (Libraries)-------------------#
import os
import socket
import time
import io
...
# %% Variable declarations and Application initialization
#--------------------------Variables declarations-----------------------------#
global first_word; first_word = "\n\n\t\t\t\t\t  Hello User"
global sys_password; sys_password = '12345yjwmx2bg6zjtSZBJUtPzf20xVXamjdfmpCkqm8=' # key used by the application to encrypt and decrypt the database file to keep it secure.
global server_address; server_address = "18.220.204.177" #Address of syncronization server
global server_port; server_port = 9999 #Port of syncronization server
global host_address; host_address = "localhost" #Application run on this address in local machine
global host_port; host_port = 80 #Application run on this port in local machine
global send; send = "send"
global receive; receive = "recv"
global send_file; send_file = "enc_accounts_c.db"
global recv_file; recv_file = "enc_accounts_c.db"
...
class Encrypter():
    def key_create(self):
        ...
        passw = 'yjwmx2bg6zjtSZBJUtPzf20xVXamjdfmpCkqm8='
        password = "12345"
...
def connect_server(act):
    try:
        ...
        if str(info) == send:
            ...
            file.close()
            c.close()
            print("\tData has been synchronized to server successfully..")         
        elif str(info) == receive:
            filename = send_file
            ...
            file.write(data)
            file.close() 
            print("\tData has been synchronized from server successfully..")
    except:
        print("\tUnable to connect to Server,..")
        print("\t Check your internet connection.")
        print("\n\tUsing the local saved file,..")
        print("\tyou may not see updated transactions..")
        time.sleep(7)
...
def exit_app():
    ...
    #%% Flask Function declarations
#----------------------Flask App functions declaration------------------------#
@app.route('/')
@app.route("/dashboard")
def dashboard():
...
def viewcustomer(cust_id=None):
    ...
    if session['usert']=="executive":
        if request.method == "POST":
            ...
            if data is not None:return render_template('viewcustomer.html', viewcustomer=True, data=data)
    ...
    if session['usert']=="executive":
        if cust_id is not None:
            if request.method != "POST":
                ...
                else:flash('Customer is deactivated or not present in database.','warning')
            else:
                cust_id = int(cust_id)
                name = request.form.get("name")
                address = request.form.get("address")
                ...
                if result is not None :
                    ...
                    db.add(temp)
                    db.commit()
                    flash("Customer data are updated successfully.","success")
                else:flash('Invalid customer Id. Please, check customer Id.','warning')
    return redirect(url_for('viewcustomer'))
    ...
    if session['usert']=="executive":
        if cust_id is not None:
            ...
            if result is not None :
                ...
                db.add(temp)
                db.commit()
                flash("Customer is deactivated.","success")
                return redirect(url_for('dashboard'))
            else:flash(f'Customer with id : {cust_id} is already deactivated or not present in database.','warning')
    return redirect(url_for('viewcustomer'))
...
def customerstatus():
    ...
    if session['usert']=="executive":
        ...
        if data:return render_template('customerstatus.html',customerstatus=True , data=data)
        else:flash('No data found.','danger')
    return redirect(url_for('dashboard'))
        ...
        if request.method == "POST":
            ...
            if result is not None :
                ...
                if result is None:
                    ...
                    if result == 0 :query = Accounts(acc_id=360110000,acc_type=acc_type,balance=amount,cust_id=cust_id,status='active',message=message,last_update=datetime.datetime.now())
                    else:query = Accounts(acc_type=acc_type,balance=amount,cust_id=cust_id,status='active',message=message,last_update=datetime.datetime.now())
                    db.add(query)
                    db.commit()
                    if query.acc_id is None:flash("Data is not inserted! Check you input.","danger")
                    else:
                        flash(f"{query.acc_type} account is created with customer ID : {query.acc_id}.","success")
                        return redirect(url_for('dashboard'))
                else:flash(f'Customer with id : {cust_id} has already {acc_type} account.','warning')
            else:flash(f'Customer with id : {cust_id} is not present in database.','warning')
...
@app.route("/viewaccount/<acc_id>" , methods=["GET", "POST"])
def viewaccount():
    if 'user' not in session:return redirect(url_for('login'))        
    if session['usert']=="executive" or session['usert']=="teller" or session['usert']=="cashier":
        if request.method == "POST":
            ...
            if data:return render_template('viewaccount.html', viewaccount=True, data=data)
...
def viewaccountstatus():
    ...
    if session['usert']=="executive":
        ...
        if data:return render_template('viewaccountstatus.html', viewaccountstatus=True, data=data)
        else:flash("Accounts are not found!", 'danger')
...
def deposit(acc_id=None):
    ...
    if session['usert']=="teller" or session['usert']=="cashier":
        if acc_id is None:return redirect(url_for('viewaccount'))
        else:
            if request.method == "POST":
                ...
                if data is not None:
                    ...
                    db.add(temp)
                    db.commit()
                else:flash(f"Account not found or Deactivated.",'danger')
            else:
                data = db.execute("select * from accounts where acc_id = :a",{"a":acc_id}).fetchone()
                if data is not None:return render_template('deposit.html', deposit=True, data=data)
                else:flash(f"Account not found or Deactivated.",'danger')
    return redirect(url_for('dashboard'))
    ...
    if session['usert']=="teller" or session['usert']=="cashier":
        if acc_id is None:return redirect(url_for('viewaccount'))
            ...
            if request.method == "POST":
                ...
                data = db.execute("select * from accounts where acc_id = :a",{"a":acc_id}).fetchone()
                if data is not None:return render_template('withdraw.html', deposit=True, data=data)
                else:flash(f"Account not found or Deactivated.",'danger')
    return redirect(url_for('dashboard'))
...
def transfer(cust_id=None):
    ...
    if session['usert'] == "executive":
        ...
        return redirect(url_for('dashboard'))
    if session['usert']=="teller" or session['usert']=="cashier" :
        if cust_id is None:return redirect(url_for('viewaccount'))
        else:
            if request.method == 'POST':
                ...
                if src_type != trg_type:
                    ...
                    if src_data is not None and trg_data is not None:
                        ...
                        else:flash("Insufficient amount to transfer.","danger")
                    else:flash("Accounts not found","danger")
                else:flash("Can't Transfer amount to same account.",'warning')
            else:
                data = db.execute("select * from accounts where cust_id = :a",{"a":cust_id}).fetchall()
                if data and len(data) == 2:return render_template('transfer.html', deposit=True, cust_id=cust_id)
                else:
...
def statement():
    ...
    if session['usert']=="teller" or session['usert']=="cashier":
        if request.method == "POST":
            ...
            if flag=="red":data = db.execute("SELECT * FROM (SELECT * FROM transactions where acc_id=:d ORDER BY trans_id DESC LIMIT :l)Var1 ORDER BY trans_id ASC;", {"d": acc_id,"l":number}).fetchall()
            else:data = db.execute("SELECT * FROM transactions WHERE acc_id=:a between DATE(time_stamp) >= :s AND DATE(time_stamp) <= :e;",{"a":acc_id,"s":start_date,"e":end_date}).fetchall()
            if data:return render_template('statement.html', statement=True, data=data, acc_id=acc_id)
            else:
        ...
        if acc_id is not None:
            ...
            if data:
                if ftype is None:
                    ...
                    pdf.cell(page_width, 0.0, "Bank Management System", align='C')
                    pdf.cell(page_width, 1.0, "Bank Management System", align='C')
                    pdf.ln(10)
                    ...
                    return response
            else:flash("Invalid account Id",'danger')
        else:flash("Please, provide account Id",'warning')
    return redirect(url_for('dashboard'))
    ...
    if request.method == "POST":
        ...
        if result is not None:
            if bcrypt.check_password_hash(result['password'], passw) is True:
                ...
                return redirect(url_for('dashboard'))
        flash("Sorry, Username or password not match.","danger")
    return render_template("adminlogin.html", login=True)
    ...
    if request.method == "POST":
        ...
        if result is not None:
            if bcrypt.check_password_hash(result['password'], passw) is True:
                ...
                return redirect(url_for('dashboard'))
        flash("Sorry, Username or password not match.","danger")
    return render_template("customerlogin.html", login=True)
...
def shutdown():
    db.close()
    shutdown_server()
    flash("You have stopped the application, Please close this tab","success")
    return render_template("home.html", home=True)
...
def main():
    ...
    encryptor = Encrypter()
    decryptor = Decrypter()
    loaded_key = bytes(sys_password, 'utf-8')
    #connect_server("recv")
    try:
        print("\n\tDecrypting the secured Database file...")
        decryptor.file_decrypt(loaded_key, 'enc_database.db', 'database.db')
        os.remove("enc_database.db")
    except:
        print("\n\tError in Database file...")
        print("\tFailed to decrypt Database file...")
    app.secret_key = 'super_secret_key'
    print("\n\tApplication Started...")
    webbrowser.open_new("http://"+ host_address +":" + str(host_port))
    app.run(host = host_address, port = host_port, debug=False)
    print("\n\n\tEncrypting the Database file and saving the Secured file...")
    encryptor.file_encrypt(loaded_key, 'database.db', 'enc_database.db')
    #connect_server("send")
    exit_app()
#--------------------------------Main loop------------------------------------#
